 ...
 /** Obtention d'une liste de mots a partir de la constante CHAPITRE2 **/
    ...
    * a chaque mot du CHAPITRE2, est associe son nombre d'occurrence
    ...
    public static Map<String,Integer> occurrencesDesMots(List<String> liste){
      ...
      List<String> list = listeDesMots();

      for(int i =0; i < list.size(); i++){
         Integer res = table.get(list.get(i)); // On essaie d'obtenir la clé dans la hashMap
         if(res == null) table.put(list.get(i),1); // S'il n'existe pas on l'ajoute à la HashMap
         else table.put(list.get(i),res+1);   // Sinon on ajoute +1 à l'occurence
        }
      return table;
    }
