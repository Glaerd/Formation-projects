...
/* construire l'AST du programme "WhileL style"  est à construire ci dessous
  ...
  ftp;
  afficher(fact);
remarque : on calcule ici fact(n) i.e. n est une donnée fournie au moment de l'exécution,
d'où le constructeur suivant
*/
  ...
  private Instruction ast= new Sequence(new Affectation( xxx, n ),new Sequence(new Affectation(fact,un),new TantQue(new Non(new Egal(xxx, zero)), new Sequence(new Affectation(fact, new Multiplication(fact,xxx)),new Affectation(xxx,new Soustraction(xxx,un))))));
