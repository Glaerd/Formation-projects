...
void demand_SC(int nbMachines, int machine, char* argv[], int s_ecoute, int sortSC) {

	int j;
	double temp1 = DBL_MAX;
	for(j = 0; j < nbMachines; j++) {
	
		if(sect.dliste[j].order < temp1){
			temp1 = sect.dliste[j].order;
		}
	
	}
	for(j = 0; j < nbMachines; j++) {
		if(j != machine) {
			if(sect.dliste[j].order == DBL_MAX || temp1 != sect.dliste[j].order){
				...
				if(connect(client, (struct sockaddr*)&sock_add, size_sock_add) == -1) {
					...
					double order;
					if(sortSC != 1) order = sect.dliste[machine].order;
					else order = DBL_MAX;
...
void checkAccords(int s_ecoute,int argc,int sitePos, int NSites, int sortSC){
...
int main(int argc, char* argv[]) {
	...
	for(l = 0; l < 1000; l++) {
		...
		double o1,o2,o3;
		if(sect.dliste[0].order == DBL_MAX) o1 = -1;
		else o1 = sect.dliste[0].order;
		if(sect.dliste[1].order == DBL_MAX) o2 = -1;
		else o2 = sect.dliste[1].order;
		if(sect.dliste[2].order == DBL_MAX) o3 = -1;
		else o3 = sect.dliste[2].order;
		
		if(demandeSect == 0 && inSect == 0) printf("%d -> %fs - 0 \t Sect Orders - {%f,%f,%f}\n", l, clock, o1, o2, o3);
		else if(demandeSect == 1 && inSect == 0) printf("%d -> %fs - 1 \t Sect Orders - {%f,%f,%f}\n", l, clock, o1, o2, o3);
		else if(demandeSect == 0 && inSect == 1) printf("%d -> %fs - 2 \t Sect Orders - {%f,%f,%f}\n", l, clock, o1, o2, o3);
		else printf("X\n");
		...
		if(demandeSect == 0) {
			if(t < 1) {
				if(inSect == 0) {
					...
					demand_SC(argc - 2, sitePos, argv, s_ecoute, sortSC);
					demandeSect = 1;
				} else if(inSect == 1) {
					sort_SC(sitePos);
					sortSC = 1;
		...
		if(inSect != 1 && sortSC != 1){
			accepteDemand(s_ecoute,sock_add,sock_add_dist,size_sock,argv,sitePos);
		}
		
		if(demandeSect == 1 || sortSC == 1) checkAccords(s_ecoute,argc,sitePos,NSites, sortSC);
		
		if(sortSC == 1) sect.dliste[sitePos].order = DBL_MAX;
	}
