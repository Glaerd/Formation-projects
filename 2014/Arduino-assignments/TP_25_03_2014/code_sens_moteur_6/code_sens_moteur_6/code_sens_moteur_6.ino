...
void loop()
{
  ...
  if (buttons) { // Si un bouton est appuyé
    ...
    if (buttons && BUTTON_RIGHT) { // Si un bouton est appuyé et qu'il s'agit de BUTTON_RIGHT
      ...
      cmpt += 20; // La valeur side est à 1
    }
  ...
  if( side == 3 && cmpt < 0) // Si side est à 3 (Donc si on a appuyé sur BUTTON_SELECT) 
                                  //et si la valeur cmpt est inférieure à 0 
  {
    cmpt += 5; // Incrémente la valeur cmpt de 5
    delay(25); // Attend 25 ms
  }
  else if( side == 3 && cmpt > 0) // Si side est à 3 (Donc si on a appuyé sur BUTTON_SELECT) 
                                  //et si la valeur cmpt est supérieure à 0 
  {
 ...
 if( cmpt > 0 ) // Si la valeur cmpt est supérieure à 0
  {
    analogWrite( S2, 0 ); // Envoie 5V sur les sorties 2-3 pendant 0 "sous-périodes" 
                          //de la période totale du PWM
    analogWrite( S1, cmpt ); // Envoie 5V sur les sorties 1-4 pendant 255 "sous-périodes" 
                              //de la période totale du PWM
  }
 else if(cmpt < 0 ) // Si la valeur cmpt est inférieure à 0
  {
    analogWrite( S1, 0 ); // Envoie 5V sur les sorties 1-4 pendant 0 "sous-périodes" 
                          //de la période totale du PWM
    analogWrite( S2, (-1) * cmpt ); // Envoie 5V sur les sorties 2-3 pendant 255 "sous-périodes" 
                            //de la période totale du PWM
  }
 else // Sinon
  {
    analogWrite( S1, 255 ); // Envoie 5V sur les sorties 1-4 pendant 255 "sous-périodes" 
                            //de la période totale du PWM
    analogWrite( S2, 255 ); // Envoie 5V sur les sorties 2-3 pendant 255 "sous-périodes" 
                            //de la période totale du PWM
  }
}
