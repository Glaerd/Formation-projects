...
void loop()
{
  ...
  if (buttons) { // Si un bouton est appuyé
    ...
    if (buttons & BUTTON_RIGHT) { // Si un bouton est appuyé et qu'il s'agit de BUTTON_RIGHT
      if (cmpt > 0) lcd.print("To The Right "); // Ecrit "To The Right" sur la première ligne
      else if (cmpt < 0) lcd.print("To The Left ");
      else if (cmpt == 0) lcd.print("Choose a Side ");
      side = 0;
      if(cmpt < 250) cmpt += 10; // 
    }
    else if (buttons & BUTTON_LEFT) { // Si un bouton est appuyé et qu'il s'agit de BUTTON_LEFT
      if (cmpt < 0) lcd.print("To The Left "); // Ecrit "To The Left" sur la première ligne
      else if (cmpt > 0) lcd.print("To The Right ");
      else if (cmpt == 0) lcd.print("Choose a Side ");
      side = 0;
      if (cmpt > -250) cmpt -= 10; // 
    }
